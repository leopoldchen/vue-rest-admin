<template>
  <div v-loading="listLoading" class="table-container">
    <el-table ref="multipleTable" :data="list" element-loading-text="Loading..." border fit highlight-current-row style="width: 100%" @sort-change="handleSort" @selection-change="handleSelectionChange">
      ...
      <el-table-column v-for="col in attrs" :key="col.alias || col.name" align="center" :prop="col.name" :label="i18n(col.alias || col.name)" :width="col.width" sortable="custom">
      ...
      <el-table-column align="center" :width="actionAreaWidth" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button v-for="op in canActions" :key="op.name" size="mini" :type="op.button" @click="handleAction(op.name, scope.row)">{{ op.title() }}</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
